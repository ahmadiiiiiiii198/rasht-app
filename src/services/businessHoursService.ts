...
class BusinessHoursService {
    ...
    async isBusinessOpen(checkTime?: Date): Promise<BusinessHoursResult> {
        ...
        // Check if current time is within business hours (lunch OR dinner)
        const currentTime = this.formatTime(now);

        // Check lunch hours first (if enabled)
        const isWithinLunchHours = todayHours.lunchIsOpen && todayHours.lunchOpenTime && todayHours.lunchCloseTime
            ? this.isTimeWithinRange(currentTime, todayHours.lunchOpenTime, todayHours.lunchCloseTime)
            : false;

        // Check dinner hours
        const isWithinDinnerHours = this.isTimeWithinRange(currentTime, todayHours.openTime, todayHours.closeTime);

        // Business is open if within either lunch OR dinner hours
        const isWithinHours = isWithinLunchHours || isWithinDinnerHours;

        console.log('🕒 [BusinessHours] Time check:', {
            currentTime,
            lunchHours: todayHours.lunchIsOpen ? `${todayHours.lunchOpenTime}-${todayHours.lunchCloseTime}` : 'Closed',
            dinnerHours: `${todayHours.openTime}-${todayHours.closeTime}`,
            isWithinLunchHours,
            isWithinDinnerHours,
            isWithinHours
        });
        ...
        if (isWithinHours) {
            const currentPeriod = isWithinLunchHours ? 'pranzo' : 'cena';
            console.log(`✅ [BusinessHours] Business is OPEN (${currentPeriod})`);
            return {
                isOpen: true,
                message: `Siamo aperti per ${currentPeriod}! Puoi effettuare il tuo ordine.`,
                todayHours
            };
        } else {
            ...
            // Build hours message
            let hoursMessage = '';
            if (todayHours.lunchIsOpen && todayHours.lunchOpenTime && todayHours.lunchCloseTime) {
                hoursMessage = `Pranzo: ${todayHours.lunchOpenTime}-${todayHours.lunchCloseTime}, Cena: ${todayHours.openTime}-${todayHours.closeTime}`;
            } else {
                hoursMessage = `Cena: ${todayHours.openTime}-${todayHours.closeTime}`;
            }

            return {
                isOpen: false,
                message: `Siamo chiusi. Orari di oggi: ${hoursMessage}`,
                nextOpenTime,
                todayHours
            };
        }
