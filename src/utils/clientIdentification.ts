...
const getClientIPAddress = async (): Promise<string> => {
    try {
        ...
        const browserFingerprint = btoa(`${navigator.userAgent}_${window.screen.width}x${window.screen.height}_${navigator.language}_${Intl.DateTimeFormat().resolvedOptions().timeZone}`).replace(/[^a-zA-Z0-9]/g, '').substring(0, 16);
        ...
        const browserFingerprint = btoa(`${navigator.userAgent}_${window.screen.width}x${window.screen.height}_${navigator.language}`).replace(/[^a-zA-Z0-9]/g, '').substring(0, 16);
